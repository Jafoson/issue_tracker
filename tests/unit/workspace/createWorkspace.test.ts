...
mock.module("@/lib/workspace-defaults", () => ({
  ...
  DEFAULT_ROLES: [{ id: "role-admin" }, { id: "role-member" }],
}));
...
const mockWorkspaceFindUnique = db.workspace.findUnique as any;
...
describe("createWorkspace()", () => {
  beforeEach(() => {
    ...
    mockTransaction.mockImplementation(async (fn: any) => { await fn(mockTx); });
  });

  describe("Auth-Prüfung", () => {
    it("gibt Fehler zurück wenn User nicht eingeloggt ist", async () => {
      mockGetSession.mockResolvedValue(null);
      const result = await createWorkspace(makeFormData({ name: "My WS", slug: "my-ws" }));
  ...
  describe("Validierung", () => {
    ...
    it("gibt Fehler zurück wenn Slug fehlt", async () => {
      const result = await createWorkspace(makeFormData({ name: "My Workspace" }));
      expect(result).toEqual({ error: "Name and slug are required." });
    });

    it("gibt Fehler zurück wenn Slug Großbuchstaben enthält", async () => {
      const result = await createWorkspace(makeFormData({ name: "My Workspace", slug: "My-Workspace" }));
    ...
    it("gibt Fehler zurück wenn Slug Sonderzeichen enthält", async () => {
      const result = await createWorkspace(makeFormData({ name: "My Workspace", slug: "my_workspace!" }));
      expect(result).toEqual({ error: "Slug may only contain lowercase letters, numbers, and hyphens." });
    });
  ...
  describe("Erfolgreiche Workspace-Erstellung", () => {
    ...
    it("gibt redirectTo zurück nach erfolgreicher Erstellung", async () => {
      const result = await createWorkspace(makeFormData({ name: "My Workspace", slug: "my-workspace", locale: "de" }));
      expect(result).toMatchObject({ redirectTo: expect.any(String) });
    });

    it("leitet zum Board weiter mit korrektem Workspace-Slug", async () => {
      const result = await createWorkspace(makeFormData({ name: "My Workspace", slug: "my-workspace", locale: "de" }));
      expect((result as { redirectTo: string }).redirectTo).toContain("/de/w/my-workspace/board/");
    });

    it("erstellt Workspace in einer Transaktion", async () => {
      await createWorkspace(makeFormData({ name: "My Workspace", slug: "my-workspace" }));
      expect(mockTransaction).toHaveBeenCalledTimes(1);
    });

    it("erstellt den User als Admin-Mitglied", async () => {
      await createWorkspace(makeFormData({ name: "My Workspace", slug: "my-workspace" }));
    ...
    it("erstellt Standard-Statuses, Prioritäten, Typen und Rollen", async () => {
      await createWorkspace(makeFormData({ name: "My Workspace", slug: "my-workspace" }));
      ...
      expect(mockTx.workspaceRole.createMany).toHaveBeenCalled();
    });
