...
const mockUserCreate = db.user.create as any;
...
describe("register()", () => {
  ...
  describe("Validierung", () => {
    it("gibt Fehler zurück wenn Name fehlt", async () => {
      const result = await register(makeFormData({ email: "user@example.com", password: "password123" }));
      expect(result).toEqual({ error: "All fields are required." });
    });

    it("gibt Fehler zurück wenn Email fehlt", async () => {
      const result = await register(makeFormData({ name: "Test User", password: "password123" }));
      expect(result).toEqual({ error: "All fields are required." });
    });

    it("gibt Fehler zurück wenn Passwort fehlt", async () => {
      const result = await register(makeFormData({ name: "Test User", email: "user@example.com" }));
      expect(result).toEqual({ error: "All fields are required." });
    });

    it("gibt Fehler zurück wenn Passwort zu kurz ist (< 8 Zeichen)", async () => {
      const result = await register(makeFormData({ name: "Test User", email: "user@example.com", password: "short" }));
      expect(result).toEqual({ error: "Password must be at least 8 characters." });
    });

    it("akzeptiert Passwort mit genau 8 Zeichen", async () => {
      const result = await register(makeFormData({ name: "Test User", email: "user@example.com", password: "12345678" }));
      expect(result).not.toMatchObject({ error: "Password must be at least 8 characters." });
    });

    it("gibt Fehler zurück bei ungültigem Email-Format", async () => {
      const result = await register(makeFormData({ name: "Test User", email: "not-an-email", password: "password123" }));
    ...
    it("gibt Fehler zurück wenn Email bereits registriert ist", async () => {
      ...
      const result = await register(makeFormData({ name: "Test User", email: "existing@example.com", password: "password123" }));
  ...
  describe("Erfolgreiche Registrierung", () => {
    it("leitet nach der Registrierung zu create-workspace weiter", async () => {
      const result = await register(makeFormData({
        name: "Test User",
