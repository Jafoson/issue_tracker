...
export function CommandPalette({ open, onClose }: CommandPaletteProps) {
  ...
  const issueHits = searchIssues
    .filter((i) => {
      ...
      return i.title.toLowerCase().includes(lq) || identifier.toLowerCase().includes(lq);
    })
    .slice(0, 6);

  type ResultItem =
    | { kind: "nav";   href: string; label: string; icon: string }
    | { kind: "issue"; id: string; title: string; status: string; identifier: string };

  const results: ResultItem[] = [
    ...allNavHits.map((e) => ({ kind: "nav" as const, href: e.href, label: e.label(t), icon: e.icon })),
    ...issueHits.map((i) => ({
      kind: "issue" as const, id: i.id, title: i.title, status: i.status,
  ...
  return createPortal(
    <div className="orbit-overlay" onClick={onClose}>
      <div className="orbit-cmd" onClick={(e) => e.stopPropagation()}>
            ...
            <>
              ...
              {allNavHits.map((e, idx) => (
                <button key={e.href}
                  ...
                  onClick={() => select({ kind: "nav", href: e.href, label: e.label(t), icon: e.icon })}>
              ...
              {issueHits.map((i, idx) => {
                ...
                return (
                  <button key={i.id}
                    ...
                    onClick={() => select({ kind: "issue", id: i.id, title: i.title, status: i.status, identifier })}>
                    <StatusIcon status={i.status} size={15} />
                    <span className={styles.issueTitle}>{i.title}</span>
                    <span className="faint mono" style={{ fontSize: 11, marginLeft: "auto" }}>{identifier}</span>
                  </button>
                );
              })}
            </>
        ...
        <div className={styles.footer}>
          ...
          <span className="kbd" style={{ marginLeft: 8 }}>↵</span> {t.palette.select}
          <span className="kbd" style={{ marginLeft: 8 }}>ESC</span> {t.palette.close}
        </div>
      </div>
    </div>,
    document.body,
