      ...
      <div style={{ maxHeight: 240, overflowY: "auto", margin: "0 -1px" }}>
        {filtered.map((l) => (
          <div
            ...
            onClick={() => { onPick(l.id); if (!keepOpen) onClose(); }}
          >
            <input
              type="checkbox"
        ...
        {filtered.length === 0 && q.trim() && (
          <div style={{ display: "flex", flexDirection: "column", gap: 2, padding: "2px 0" }}>
            <div className="menu-item" onClick={() => setPending({ name: q.trim(), scope: "project" })}>
              <Icon icon="lucide:plus" width={14} height={14} style={{ flexShrink: 0 }} />
              <span>„{q.trim()}" in <strong>{projectName}</strong> anlegen</span>
            </div>
            <div className="menu-item" onClick={() => setPending({ name: q.trim(), scope: "workspace" })}>
              <Icon icon="lucide:plus" width={14} height={14} style={{ flexShrink: 0 }} />
